...
- Fixed compatibility issues with Claude Desktop:
  - Modified response format in `index.ts` to ensure `content` is always an array of objects with `type` and `text` properties
  - Fixed the "s.content.map is not a function" error that occurred when using the MCP server with Claude Desktop
- Fixed TypeScript compilation issues:
  - Updated `tsconfig.json` to properly emit JavaScript files by setting `emitDeclarationOnly: false` and `noEmit: false`
  - Ensured compatibility with the extended TypeScript configurations from @sanity/pkg-utils
- Added direct execution scripts to package.json:
  - Added `dev:tsx` script to run src/index.ts directly with tsx
  - Added `start:tsx` script for production use with tsx
  - Added `test:tsx` script to run test.ts server directly
  - Simplified server execution without requiring a full build
