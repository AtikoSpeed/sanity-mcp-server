 ...
 */
export class ContextToolProvider {
    /**
        ...
        return [
            {
                ...
                description: 'IMPORTANT: Call this tool first to get initial context and usage instructions for this MCP server. ' +
                ...
                handler: async () => {
                    ...
                    try {
                        ...
                        const documentTypes = await schemaController.listSchemaTypes(config.projectId, config.dataset || 'production', { allTypes: false });
                        // Get active releases
                        const releasesResult = await releasesController.listReleases(config.projectId, config.dataset || 'production');
                        // Filter to only active releases
                        const activeReleases = releasesResult.releases.filter((release) => !release.archivedAt && !release.discardedAt);
                        return {
                            message: 'Welcome to the Sanity MCP Server!',
                            instructions: 'You can use this server to interact with Sanity content. ' +
                                'Start by exploring the schema to understand the content model.',
                            projectId: config.projectId,
                            dataset: config.dataset || 'production',
                            embeddingsIndices: embeddingsIndices.map((index) => ({
                            ...
                            note: "The above information provides context about the Sanity project you're working with. " +
