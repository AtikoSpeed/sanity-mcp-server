...
export async function listEmbeddingsIndices({ projectId = config.projectId || process.env.SANITY_PROJECT_ID, dataset = config.dataset || process.env.SANITY_DATASET } = {}) {
    try {
        // Ensure we have the necessary info
        if (!projectId || !dataset) {
            throw new Error('Project ID and Dataset name are required. ' +
 ...
 */
function validateSearchParameters(query, indexName, projectId, dataset) {
    if (!query) {
        throw new Error('Query parameter is required for semantic search');
    }
    if (!indexName) {
        throw new Error('indexName parameter is required for semantic search. ' +
 ...
 */
function createSearchPayload(query, maxResults, types) {
    return {
        query: query,
        limit: maxResults,
...
function processSearchResults(rawResults) {
    ...
    if ('hits' in rawResults && Array.isArray(rawResults.hits)) {
        return {
            ...
            total: rawResults.total || rawResults.hits.length
        };
    }
 ...
 */
async function handleEmbeddingsApiError(response, indexName) {
    if (response.status === 401 || response.status === 403) {
        throw new Error("Authentication failed: Your Sanity token is invalid or doesn't have access to embeddings.");
    }
 ...
 */
export async function semanticSearch(query, { indexName, maxResults = 10, types = [], projectId = config.projectId || process.env.SANITY_PROJECT_ID, dataset = config.dataset || process.env.SANITY_DATASET } = {}) {
    try {
        ...
        if (!embeddingsResponse.ok) {
            await handleEmbeddingsApiError(embeddingsResponse, indexName);
        }
