...
import { getDocumentContent, normalizeBaseDocId, normalizeDocumentIds } from '../utils/documentHelpers.js';
...
function validateApiVersion() {
    ...
    if (!isSufficientApiVersion(currentVersion, REQUIRED_API_VERSION)) {
        throw new Error(`API version ${currentVersion} is outdated. Please update to version ${REQUIRED_API_VERSION} or later ` +
 ...
 */
function validateReleaseParameters(_releaseId, // Prefix with underscore to indicate it's intentionally unused
options) {
 ...
 */
function createReleaseMetadata(releaseId, title, options) {
    return {
 ...
 */
function handleReleaseCreationError(error, releaseId) {
    ...
    if (error.message?.includes('API version')) {
        ...
        throw new Error('Failed to create release: The Content Releases feature might not be enabled for this project ' +
 ...
 */
export async function createRelease(projectId, dataset, releaseId, title, options) {
    try {
 ...
 */
function createVersionAction(releaseId, documentId, attributes) {
 ...
 */
async function processDocumentForRelease(client, releaseId, documentId, content) {
    const baseDocId = normalizeBaseDocId(documentId);
...
export async function addDocumentToRelease(projectId, dataset, releaseId, documentIds, content) {
    try {
        // Check API version firs
        validateApiVersion();
        ...
        for (const docId of normalizedIds) {
            try {
                ...
                errors.push(`Document ID ${docId}: ${documentError instanceof Error ? documentError.message : String(documentError)}`);
            }
        }
 ...
 */
export async function removeDocumentFromRelease(projectId, dataset, releaseId, documentIds) {
    try {
        validateApiVersion();
        // Normalize to always work with an array of document IDs
        const normalizedIds = normalizeDocumentIds(documentIds);
...
export async function publishRelease(projectId, dataset, releaseId) {
    try {
        ...
        if (documents.documentCount > 50) {
            throw new Error(`Release contains ${documents.documentCount} documents, which exceeds the 50 document limit`);
        }
 ...
 */
export async function listReleases(projectId, dataset) {
    try {
        // Check API version firs
        validateApiVersion();
        const client = createSanityClient(projectId, dataset);
 ...
 */
export async function getRelease(projectId, dataset, releaseId) {
    try {
        // Check API version firs
        validateApiVersion();
        const client = createSanityClient(projectId, dataset);
 ...
 */
export async function updateRelease(projectId, dataset, releaseId, updateData) {
    try {
        // Check API version firs
        validateApiVersion();
 ...
 */
export async function scheduleRelease(projectId, dataset, releaseId, publishAt) {
    try {
        // Check API version firs
        validateApiVersion();
 ...
 */
export async function unscheduleRelease(projectId, dataset, releaseId) {
    try {
        // Check API version firs
        validateApiVersion();
 ...
 */
export async function archiveRelease(projectId, dataset, releaseId) {
    try {
        // Check API version firs
        validateApiVersion();
 ...
 */
export async function unarchiveRelease(projectId, dataset, releaseId) {
    try {
        // Check API version firs
        validateApiVersion();
 ...
 */
export async function deleteRelease(projectId, dataset, releaseId) {
    try {
        // Check API version firs
        validateApiVersion();
