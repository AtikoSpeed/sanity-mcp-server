    ...
    try {
        ...
        try {
            ...
            if (readError instanceof Error && 'code' in readError && readError.code === 'ENOENT') {
                throw new Error(`Schema file not found for project ${projectId} and dataset ${dataset}. ` +
 ...
 */
export async function getSchemaForType(projectId, dataset = 'production', typeName, options = {}) {
    try {
        // Get the full schema
        ...
        throw new Error(`Failed to get schema for type ${typeName}: ${error instanceof Error ? error.message : String(error)}`);
    }
}
/**
 * Process a reference field and add referenced types to the collection
 */
function processReferenceField(refField, allTypes, referencedTypes, processedTypes) {
    if (!refField.to) {
        return;
    }
 ...
 */
function processArrayTypeItem(arrayType, allTypes, referencedTypes, processedTypes) {
 ...
 */
function processArrayField(arrayField, allTypes, referencedTypes, processedTypes) {
    if (!arrayField.of) {
        return;
    }
 ...
 */
function processField(field, allTypes, referencedTypes, processedTypes) {
 ...
 */
function processFields(type, allTypes, referencedTypes, processedTypes) {
 ...
 */
function findReferencedTypes(typeSchema, allTypes) {
