import { applyPatchOperations, getDocumentContent, normalizeBaseDocId, normalizeDocumentIds, normalizeDraftId } from '../utils/documentHelpers.js';
 ...
 */
function prepareDocumentForCreation(document) {
 ...
 */
async function createMultipleDocuments(client, documents, options) {
    ...
    return {
        ...
        ids: results.results.map((res) => res.id)
    };
}
/**
 * Creates a single documen
 */
async function createSingleDocument(client, document, options) {
    ...
    if (options?.ifExists === 'ignore' && preparedDoc._id) {
        ...
        result = await client.createIfNotExists(preparedDoc);
    }
    else {
        // Default behavior - just create
        result = await client.create(preparedDoc);
    }
 ...
 */
export async function createDocument(projectId, dataset, documents, options) {
    try {
 ...
 */
export async function editDocument(projectId, dataset = 'production', documentId, patch) {
    try {
        const client = createSanityClient(projectId, dataset);
 ...
 */
export async function deleteDocument(projectId, dataset, documentId, options) {
    try {
        const client = createSanityClient(projectId, dataset);
        // Handle array of document IDs
        if (Array.isArray(documentId)) {
 ...
 */
function prepareDocumentIdForDeletion(id) {
    // Ensure document ID doesn't already have 'drafts.' prefix
    const baseDocId = id.replace(/^drafts\./, '');
    const draftId = `drafts.${baseDocId}`;
 ...
 */
function setupDeleteTransaction(transaction, documentIds, additionalDrafts) {
    const processedIds = [];
 ...
 */
async function deleteMultipleDocuments(client, documentIds, options) {
    // Process each document ID
    const transaction = client.transaction();
    const { transaction: preparedTransaction, processedIds } = setupDeleteTransaction(transaction, documentIds, options?.includeDrafts);
    // Commit the transaction
 ...
 */
function prepareDocumentForReplacement(doc) {
 ...
 */
export async function replaceDraftDocument(projectId, dataset, documents) {
    try {
        const client = createSanityClient(projectId, dataset);
        // Handle array of documents
        if (Array.isArray(documents)) {
 ...
 * @returns Result of the edit operation
 */
async function editSingleDocument(client, documentId, patch) {
 ...
 * @returns Result of the edit operation
 */
async function editMultipleDocuments(client, documentIds, patch) {
...
function patchObjToSpec(patchObj) {
 ...
 */
async function createSingleDocumentVersion(client, releaseId, documentId, content) {
 ...
 */
export async function createDocumentVersion(projectId, dataset, releaseId, documentId, content) {
    try {
        const client = createSanityClient(projectId, dataset);
        // Handle array of document IDs
        if (Array.isArray(documentId)) {
 ...
 */
function setupDiscardVersionsTransaction(client, versionIds, options) {
    const transaction = client.transaction();
 ...
 */
export async function discardDocumentVersion(projectId, dataset, versionId, options) {
    try {
        const client = createSanityClient(projectId, dataset);
 ...
 */
export async function unpublishDocumentWithRelease(projectId, dataset, releaseId, documentId) {
    try {
        const client = createSanityClient(projectId, dataset);
        // Handle array of document IDs
        if (Array.isArray(documentId)) {
 ...
 * Process document IDs
 */
export function processDocumentIds(documentId) {
