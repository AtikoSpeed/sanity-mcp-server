 ...
 */
export function validateProjectDataset(projectId, dataset) {
 ...
 */
export function validateMutations(mutations) {
    ...
    mutations.forEach((mutation, index) => {
        ...
        const hasValidOperation = ('create' in mutation ||
            'createOrReplace' in mutation ||
            'createIfNotExists' in mutation ||
            'delete' in mutation ||
            'patch' in mutation);
        if (!hasValidOperation) {
            throw new Error(`Invalid mutation at index ${index}: must contain at least one operation ` +
 ...
 */
export function validateGroqQuery(query) {
